CREATE EXTENSION IF NOT EXISTS pg_trgm; -- helps fuzzy / ILIKE searches
...
ALTER TABLE post
ADD COLUMN tsv_tile_content tsvector GENERATED ALWAYS AS to_tsvector('english', title, content) STORED;
-- ! Indexes for post
-- Purpose:
...
CREATE TABLE IF NOT EXISTS post_like (
  ...
  created_at timestamptz NOT NULL DEFAULT now(),
  ADD CONSTRAINT fk_postlike_post FOREIGN KEY (post_id) REFERENCES post(id) ON DELETE CASCADE,
